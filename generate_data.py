...
class Trajectory:
    ...
    def __init__(self, grid_size, h, pos, angle, v0, force_arg):
        ...
        # force parameters; depend on instance's class
        self.force_arg = force_arg

    @abstractmethod
    def force_fct(self, t, y, arg):
        pass
    ...
    def draw_trajectory(self, potential=False):
        ...
        if potential:
            ...
            plt.imshow(self.pot_fct(gridx, gridy[::-1, :]),
...
class Coulomb_trajectory(Trajectory):
    def __init__(self, grid_size, h, pos, angle, v0, force_arg):
        if len(force_arg) != 4:
            print("Error: wrong length of force parameters")
        super(Coulomb_trajectory, self).__init__(grid_size, h, pos,
                                                 angle, v0, force_arg)

    def pot_fct(self, x, y):
        return self.force_arg[2] / (np.sqrt((x - self.force_arg[0])**2 +
                                            (y - self.force_arg[1])**2) +
                                    self.force_arg[3])

    def force_fct(self, t, y, arg):
        r = np.sqrt((y[0] - arg[0])**2 + (y[1] - arg[1])**2)
        return [y[2], y[3], arg[2]*(y[0] - arg[0])/(r + arg[3])**3,
                arg[2]*(y[1] - arg[1])/(r + arg[3])**3]
...
class Const_trajectory(Trajectory):
    def __init__(self, grid_size, h, pos, angle, v0, force_arg):
        if len(force_arg) != 2:
            print("Error: wrong length of force parameters")
        super(Const_trajectory, self).__init__(grid_size, h, pos,
                                               angle, v0, force_arg)

    def pot_fct(self, x, y):
        return self.force_arg[0]*x + self.force_arg[1]*y

    def force_fct(self, t, y, grad):
        return [y[2], y[3], -grad[0], -grad[1]]


# # class for Gaussian hill
# class Gaussian_trajectory(Trajectory):
#     def __init__(self, grid_size, h, pos, angle, v0, force_arg):
#         # Parameters of a 2D Gaussian: mean and covariance matrix
#         if len(force_arg) != 2:
#             print("Error: wrong length of force parameters")
#         super(Coulomb_trajectory, self).__init__(grid_size, h, pos,
#                                                  angle, v0, force_arg)
...
if __name__ == '__main__':
    ...
    if fixed_start:
        ...
        fname = 'pong_fixed_start{}x{}'.format(grid[0], grid[1])
    else:
        nangles = int(100/.3)
        nstarts = int(24 * 3 * 2)
        starts = field[0]*np.random.beta(1.5, 1.5, nstarts)
        fname = 'pong_var_start{}x{}'.format(grid[0], grid[1])
    ...
    pooled_last_col = np.sum(last_col.reshape((last_col.shape[0], n_labels,
                             ...
                             axis=2)
    # if one-hot labels desired
    labels = np.floor(pooled_last_col /
                      np.expand_dims(np.max(pooled_last_col, axis=1),
                                     1)).astype(int)
    ...
    # # inspect pixels
    # samples = tile_raster_images(data[0][:4, :],
    ...
    # ang = 30.
    # coul_args = [field[0]/3, field[1]/2, c, epsilon]
    # const_args = [0., 0.]
    # test = Const_trajectory(grid, h, start_pos, ang, v0, const_args)
    # #test = Trajectory(grid, h, start_pos, ang, v0)
    # test.integrate()
