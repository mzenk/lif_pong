...
def get_performance_data(basefolder, compare_stub, data_idx):
...
def plot_agent_performance(ax, agent_dict, label=None):
    ...
    ax.errorbar(speeds, succrate, yerr=succrate_std, label=label)
...
def main(stub_list, experiment_dict):
    ...
    repl_dict = experiment_dict.pop('replacements')
    data_name = stub_dict['general']['data_name']
    data_idx = []
    # this could lead to errors if last chunk is smaller
    [data_idx.append(np.arange(s, s + chunksize))
        for s in repl_dict['start_idx']]
    ...
    for i, stub_dict in enumerate(stub_list):
        ...
        prediction, agent_result = get_performance_data(
            os.path.join(simfolder, expt_name), stub_dict, data_idx)
