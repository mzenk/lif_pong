...
import numpy as np
import sys
from util import get_data_path, make_data_folder, average_helper
...
class Pong_agent(object):
    ...
    def simulate_games(self, predictions, targets):
        ...
        """
        paddle_pos = .5 * np.sum(self.pos_range)
...
for i, speed in enumerate(speed_range):
    ...
    my_agent = Pong_agent(img_shape[0], paddle_len=lab_width, max_step=speed)
